    ...
    stage("Build CPP"){
        sh(label: "Testing CPP Code",
           script: """conan install . -if build -o "*:shared=True"
                      cmake -B ${buildPath} -DCMAKE_TOOLCHAIN_FILE=build/conan_paths.cmake -DCMAKE_POSITION_INDEPENDENT_CODE:BOOL=true -DBUILD_TESTING:BOOL=true -DCMAKE_CXX_FLAGS="-fprofile-arcs -ftest-coverage"
