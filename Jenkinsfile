...
pipeline {
    ...
    parameters {
        ...
        booleanParam(name: 'RUN_MEMCHECK', defaultValue: true, description: 'Run Memcheck. NOTE: This can be very slow.')
                                                ...
                                                tee('logs/cmake-build.log'){
                                                    sh(label: 'Compiling CPP Code',
                                                       script: '''conan install . -if build/cpp -o "*:shared=True"
                                                                  cmake -B build/cpp -Wdev -DCMAKE_BUILD_TYPE=Debug -DCMAKE_TOOLCHAIN_FILE=build/cpp/conan_paths.cmake -DCMAKE_EXPORT_COMPILE_COMMANDS:BOOL=ON -DCMAKE_POSITION_INDEPENDENT_CODE:BOOL=true -DBUILD_TESTING:BOOL=true -DCMAKE_CXX_FLAGS="-fno-inline -fno-omit-frame-pointer -fprofile-arcs -ftest-coverage -Wall -Wextra" -DMEMORYCHECK_COMMAND=$(which drmemory) -DMEMORYCHECK_COMMAND_OPTIONS="-no_callstack_use_top_fp -no_callstack_use_fp -callstack_conservative -check_uninit_blacklist libopenjp2.so.7"
